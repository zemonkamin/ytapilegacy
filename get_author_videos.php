...
if (empty($author)) {
    ...
    echo json_encode(array('error' => 'Author parameter is required'));
    exit;
}

// First, search for the channel by author name
$search_url = "https://www.googleapis.com/youtube/v3/search?part=snippet&q=" . urlencode($author) . "&type=channel&maxResults=1&key={$api_key}";
...
if (!isset($search_data['items'][0]['id']['channelId'])) {
    header('Content-Type: application/json');
...
if (!isset($channel_data['items'][0])) {
    header('Content-Type: application/json');
    echo json_encode(array('error' => 'Channel details not found'));
    exit;
}
