...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    if (query) {
      fetch(`../get_search_videos.php?apikey=<?php echo $apikey; ?>&query=${encodeURIComponent(query)}`)
        ...
        .then(data => {
          ...
          if (data.length === 0) {
            searchResults.innerHTML = '<div class="search-message">No videos found for your search query.</div>';
            return;
          }
        ...
        .catch(error => {
          ...
          document.getElementById('search-results').innerHTML = 
            '<div class="search-message">Error loading search results. Please try again later.</div>';
        });
    } else {
      document.getElementById('search-results').innerHTML = 
        '<div class="search-message">No search query provided.</div>';
    }
  });
</script>
</body>
