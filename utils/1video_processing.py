...
def get_video_with_audio_url(video_id, quality_choice, cookie_file=None):
    """Получает URL видео с аудио дорожкой с использованием yt-dlp с командой в формате:
    yt-dlp.exe --remote-components ejs:github --cookies cookies.txt -f "bestvideo[height<=1080]+bestaudio" --get-url "https://www.youtube.com/watch?v=video_id"
    """
    try:
        url = f'https://www.youtube.com/watch?v={video_id}'
        
        # Определяем максимальную высоту в зависимости от quality_choice
        if quality_choice == 'max' or quality_choice is None:
            format_filter = 'bestvideo+bestaudio'
        else:
            try:
                height = int(quality_choice)
                format_filter = f'bestvideo[height<={height}]+bestaudio'
            except ValueError:
                # Если не удалось преобразовать в число, используем стандартное значение
                print(f"[WARNING] Invalid quality choice: {quality_choice}, using default")
                format_filter = 'bestvideo[height<=1080]+bestaudio'
        
        # Формируем команду yt-dlp с указанными параметрами
        args = [
        ...
        # Добавляем файл cookies, если он существует
        if cookie_file and os.path.isfile(cookie_file):
            args = ['--cookies', cookie_file] + args
        else:
            # Пытаемся использовать cookies.txt из корня проекта
            default_cookie_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'cookies.txt')
            if os.path.isfile(default_cookie_path):
                args = ['--cookies', default_cookie_path] + args
        
        video_url = run_yt_dlp(args)
        
        if video_url:
            return video_url.strip()
        else:
            print(f"[ERROR] Failed to get video URL for video_id: {video_id}, quality: {quality_choice}")
            return None
            
    except Exception as e:
        print(f'Error in get_video_with_audio_url for video_id {video_id}, quality {quality_choice}:', e)
        return None
